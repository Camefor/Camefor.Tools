    ...
    public class ExcelHelper
    {
        ...
        public static MemoryStream ExportMemoryStream(DataTable dtSource, ExcelConfig excelConfig)
        {
            ...
            #region 右击文件 属性信息
            {
                ...
                si.ApplicationName = "xx信息"; //填加xls文件创建程序信息
        ...
        public static DataTable ExcelImport(string strFileName)
        {
            ...
            //遍历所有标头，判断哪些列是时间 日期信息。把对应列索引记录下来
            List<int> matchForTime = new List<int>();

            for (int j = 0; j < cellCount; j++)
            {
                ICell cell = headerRow.GetCell(j);
                var _cell = cell.ToString();
                _cell = Regex.Replace(_cell, @"\s", "");//去除列名中的空格，人工输入数据时错误
                dt.Columns.Add(_cell);

                //20210409 新增  处理C#与MySql数据库时间日期类型 不兼容问题
                if (_cell.Contains("日期") || _cell.Contains("时间"))
                {
                    // || _cell.Contains("开始") || _cell.Contains("结束")
                    matchForTime.Add(cell.ColumnIndex);
                }
            }
                ...
                try
                {
                    ...
                    if (row.Cells.Count >= 1)
                    {
                        for (int j = row.FirstCellNum; j < cellCount; j++)
                        {
                            ...
                            {
                                /***
                                 * 处理C#与MySql数据库时间日期类型 不兼容问题，
                                 * 从excel表格 拿数据之后 转换为DataTable之前。
                                 ***/
                                if (matchForTime.Contains(j))
                                {

                                    if (DateTime.TryParse(row.GetCell(j).ToString(), out DateTime temptime))
                                    {
                                        dataRow[j] = temptime.ToString("yyyy-MM-dd HH:mm:ss");
                                    }
                                    else
                                    {
                                        var time = row.GetCell(j).DateCellValue;
                                        dataRow[j] = time.ToString("yyyy-MM-dd HH:mm:ss");
                                    }
                                }
                                else
                                {
        ...
        /// <summary>
        /// 读取excel 支持读取多个sheet ,默认第一行为标头 
        /// </summary>
        /// <param name="afilePath">excel表格文件的物理磁盘路径</param>
        /// <returns>
        /// item1:excel表格转换为DataTable数据集<br/>
        /// item2:sheetname集合
        /// </returns>
        /// <summary>
        /// 读取excel 支持读取多个sheet ,默认第一行为标头 
        ...
        /// item2:sheetname集合
        /// </returns>
        public static Tuple<List<DataTable>, List<string>> ExcelImportMultiSheet(string afilePath)
        {
            List<DataTable> dataTables = new List<DataTable>();
            List<string> sheetNames = new List<string>();
            using (FileStream file = new FileStream(afilePath, FileMode.Open, FileAccess.Read))
            {
                List<ISheet> sheets = new List<ISheet>();

                if (afilePath.IndexOf(".xlsx") == -1)//2003
                {
                    HSSFWorkbook hssfworkbook = new HSSFWorkbook(file);
                    for (int i = 0; i < hssfworkbook.NumberOfSheets; i++)
                    {
                        var s = hssfworkbook.GetSheetAt(i);
                        sheets.Add(s);
                ...
                else//2007
                {
                    ...
                    {
                        var s = xssfworkbook.GetSheetAt(i);
                        sheets.Add(s);
                        sheetNames.Add(s.SheetName);
                    }
                }
                ...
                foreach (ISheet sheet in sheets)
                {
                    ...
                    for (int i = (sheet.FirstRowNum + 1); i <= sheet.LastRowNum; i++)
                    {
                        ...
                        for (int j = row.FirstCellNum; j < cellCount; j++)
                        {
                            var _cellValue = row.GetCell(j);
                            if (_cellValue != null)
                            {
                                if (string.IsNullOrEmpty(_cellValue.ToString()))
                                {
                                    isCanAddRow = false;
                                }
                                else
                                {
                                    if (matchForTime.Contains(j))
                                    {
                                        ...
                                        if (DateTime.TryParse(_cellValue.ToString(), out DateTime temptime))
                                        {
                                            dataRow[j] = temptime.ToDateTimeString();
                                        }
                                        else
                                        {
                                            var time = _cellValue.DateCellValue;
                                            dataRow[j] = time.ToDateTimeString();
                                        }
                                    }
                                    else
                                    {
                                        dataRow[j] = _cellValue.ToString();
                                    }
                                }
                            }
                        }
            ...
            //return dataTables;
            return Tuple.Create(dataTables, sheetNames);
        }
